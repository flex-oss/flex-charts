...
public abstract class LiveDataSeries extends PointSeries implements IProcessableOption {
    ...
    /**
     * The key under which {@link LiveDataSeries} are registered in the parent options. See
     ...
     * 
     * @param parentOptions the {@link Options} to which this series are added.
     * @param updateIntervalMs the interval in which to update the series in milliseconds.
     */
    ...
    /**
     * Adds a javascript parameter that will be passed into the {@link #update(LiveDataUpdateEvent)} method.
     *
     * @param parameterName the name of the parameter
     * @param javascriptExpression a javascript expression. The value this expression evaluates to will be transmitted
     *        to the server via AJAX and will be passed into {@link #update(LiveDataUpdateEvent)}. The javascript
     *        expression may be a function call or a literal. If it is a literal string, you have to surround the string
     *        with single quotes. Don't use double quotes in an expression since they are not escaped correctly.
    ...
    /**
     * This method is called for each update interval. It must return a point which is then added to the series on the
     * fly.
     *
     * May return null. In that case, the chart is simply not updated.
     * 
     * @param event the {@link LiveDataUpdateEvent}
     * 
     * @return the new point to add to the series. This point is added by calling Highcharts' addPoint() function.
     *         Please note that Highcharts does not evaluate all attributes of this point. Also not that to define the
     *         color of the point you have to add a Marker with the fillColor attribute defined!
     */
