...
/**
 * This processor adds drilldown functionality to a chart. It searches the given options for {@link Point}. If any
 * drilldown options are found, they are serialized in a special global javascript variable. This variable is referenced
 * in a javascript function that is inserted at {@link PlotOptions}.
 *
 * This class is not part of the public API! Use {@link DrilldownPoint}s to enable drilldown in your charts.
 *
...
public class DrilldownProcessor implements IOptionsProcessor {
    ...
    private void addDrilldownOptionsArray(OptionsProcessorContext context) {
        ...
        response.render(JavaScriptHeaderItem.forScript(
                                                       MessageFormat.format("var {0};\n var {1};",
                                                                            JS_DRILLDOWN_ARRAY_NAME,
                                                                            getDrilldownArrayName(component)),
                                                       JS_DRILLDOWN_ARRAY_NAME + "-init"));
        response.render(OnDomReadyHeaderItem
            .forScript(MessageFormat.format("{0} = {1};", getDrilldownArrayName(component),
    ...
    private void addJavascriptDependencies(IHeaderResponse headerResponse) {
        headerResponse
            .render(JavaScriptHeaderItem.forReference(new JavaScriptResourceReference(DrilldownFunction.class,
                    "drilldown.js")));
    }

    /**
     * Adds a {@link DrilldownFunction} to the {@link PlotOptions} of the given {@link Options}.
     *
