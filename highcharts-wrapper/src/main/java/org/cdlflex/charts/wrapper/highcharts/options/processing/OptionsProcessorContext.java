...
/**
 * A context object that holds some context variables that may be needed by {@link IOptionsProcessor}s.
 *
...
public class OptionsProcessorContext implements Serializable {
    ...
    private void collectInteractionFunctions(final Options options) {
        List<? extends IProcessableOption> functions =
            options.getMarkedForProcessing(InteractionFunction.PROCESSING_KEY);
    ...
    private void collectSelectionFunctions(final Options options) {
        List<? extends IProcessableOption> functions =
            options.getMarkedForProcessing(SelectionFunction.PROCESSING_KEY);
        this.selectionFunctions.addAll((List<SelectionFunction>) functions);
    }

    /**
     * Iterates recursively through all {@link DrilldownPoint}s and their drilldownOptions in the given {@link Options}
     * and adds them to the context.
     */
    @SuppressWarnings("unchecked")
    private void collectDrilldownOptions(final Options options) {
        List<? extends IProcessableOption> drilldownPoints =
            options.getMarkedForProcessing(DrilldownPoint.PROCESSING_KEY);
    ...
    /**
     * Returns true if the {@link Options} attached to this context contains the given feature.
     * 
     * @param feature the feature for which to look.
