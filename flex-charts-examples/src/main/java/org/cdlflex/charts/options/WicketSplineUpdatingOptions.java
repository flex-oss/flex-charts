...
import org.cdlflex.charts.wrapper.highcharts.options.livedata.LiveDataUpdateEvent;
...
public class WicketSplineUpdatingOptions extends ShowcaseOptions {
    ...
    public WicketSplineUpdatingOptions() {
        ...
        this.setTitle(new Title("Live random data"))
            .setSubtitle(new Title("The Y values are calculated on the server. Depending on the value,"
                + " the server creates javascript to change the color."));
        ...
        LiveDataSeries series = new LiveDataSeries(this, 1000) {
            ...
            public Point update(final LiveDataUpdateEvent event) {
                ...
                target.getHeaderResponse().render(
                                                  OnDomReadyHeaderItem.forScript(event.getJavascriptChartName()
                                                      + ".series[0].graph.attr('stroke', '"
                                                      + newColor
                                                      + "');"));
